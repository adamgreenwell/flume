...
| Repo visibility    | Public                                                 | Built open-source-ready and flipped. Being public is now load-bearing rather than incidental: SignPath Foundation's free code signing requires a publicly available codebase under an OSI-approved licence, so going private again would cost Windows signing                                                                                                                                                                                                                                                                                                                                                                                |
...
| Windows signing    | SignPath Foundation, applied 2026-09-10, pending       | Reversed. The old reasoning rested on EV clearing SmartScreen instantly, which Microsoft removed in 2024 — no certificate buys instant trust now, so nothing is worth several hundred a year. What changed the answer is that _unsigned_ reputation restarts at zero on every release and Smart App Control blocks unsigned binaries outright, which #176's updater makes worse with every version shipped. SignPath is free and needs no hardware token; the trade is that the publisher string reads SignPath Foundation, not Adam Greenwell. Azure Artifact Signing (~$120/yr, own identity) was the alternative and was declined on cost |
| Settings store     | Plain JSON file in app-data, written by Rust           | The agreed call was JSON rather than SQLite, and this is that. `tauri-plugin-store` was deliberately not used: its distinguishing feature is letting the _frontend_ read and write the store directly, which is the one thing Flume does not want — every settings change passes through Rust so the engine can apply it. See the module doc in `src-tauri/src/settings/mod.rs`                                                                                                                                                                                                                                                              |
| Per-torrent state  | `library.json`, never `settings.json`                  | `settings` holds values a user types; the library record holds what Flume knows about one torrent, keyed by info hash. The dividing line is pruning: a removal drops a library record, and nothing prunes a map in `settings.json` — so per-torrent limits kept there would accumulate the info hash of every torrent anyone ever set one on, for the life of the install. Seeding time and seed-limit overrides both live there for that reason                                                                                                                                                                                             |
...
src-tauri/
  ...
  src/policy/         pure rules over a snapshot — no Tauri types, no I/O
  src/library/        per-torrent records Flume owns, keyed by info hash
  src/egress/         which interface traffic leaves by — no Tauri types
  ...
  src/guard.rs        the egress guard's loop; the only thing that stops the engine
  tests/engine.rs     integration tests against a real librqbit Session
  tests/policy.rs     the rule engine, plus its round trip through AppState
